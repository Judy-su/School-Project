...
ggplot (data = top_countries, 
  ...
  labs(x = "Sum of Net Forest Area Change (in thousand hectares)", y = "",
       title = "15 Countries with the Largest Changes in Forest Area",
...
ggplot() +
  ...
  labs(x = "Year", y = "Total Forest Area Lost (in hectares)", 
       title = "Trend and Causes of Deforestation in Brazil",
       subtitle = "From 2001 to 2013") +
...
ggplot(data = forest_area_worst_6, aes(y = forest_area)) +
  geom_point(aes(x = total, color = "Vegetable Oil")) +
  geom_point(aes(x = processed, color = "Processed Soy Bean")) +
  geom_smooth(aes(x = total), method = "lm", formula = y~x, color = 'darkblue') +
  geom_smooth(aes(x = processed), method = "lm", formula = y~x, color = 'darkblue') +
  facet_wrap(~entity, nrow = 2, scale = 'free') +
  labs(x = "Log of Amount Produced (in tonnes)", 
       y = "Percentage of Global Forest Area", 
       title = "Impact of Production on Forest Area") +
